...
import (
	...
	"github.com/metanonia/go-metanonia/consensus/clique"
...
// PublicMetanoniaAPI provides an API to access Metanonia related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicMetanoniaAPI struct {
	b Backend
}

// NewPublicMetanoniaAPI creates a new Metanonia protocol API.
func NewPublicMetanoniaAPI(b Backend) *PublicMetanoniaAPI {
	return &PublicMetanoniaAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicMetanoniaAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicMetanoniaAPI) Syncing() (interface{}, error) {
...
// Sign calculates an Metanonia ECDSA signature for:
// keccack256("\x19Metanonia Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/metanonia/go-metanonia/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Metanonia Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/metanonia/go-metanonia/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Metanonia signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Metanonia blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Metanonia blockchain API.
...
// PublicDebugAPI is the collection of Metanonia APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Metanonia APIs exposed over the private
// debugging endpoint.
...
// Version returns the current Metanonia protocol version.
