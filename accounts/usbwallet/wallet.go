...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Metanonia
...
type wallet struct {
	...
	deriveChain     metanonia.ChainStateReader // Blockchain state reader to discover used account with
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived Metanonia account
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain metanonia.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
//
// Note, if the version of the Metanonia application running on the Ledger wallet is
